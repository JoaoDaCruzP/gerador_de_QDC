...
def main(page):
    ...
    def salvar_arquivo(event):
        ...
        file_picker.save_file(file_name=f'Quadro de cargas {nome_input.value}.xlsx')
        
    def salvar_no_caminho(event):
            # Verifica se o usuário selecionou um caminho para salvar
        if event.path:  # Use 'event.path' ao invés de 'event.result'
            try:
                # Salva o arquivo no caminho escolhido
                workbook.save(event.path)  # Acesse o caminho via event.path
                feedback_text.value = f"Arquivo salvo com sucesso em {event.path}"
                feedback_text.color = ft.Colors.GREEN
            except Exception as e:
                feedback_text.value = f"Erro ao salvar o arquivo: {e}"
                feedback_text.color = ft.Colors.RED
        else:
            feedback_text.value = "Nenhum caminho selecionado."
            feedback_text.color = ft.Colors.RED
    ...
    file_picker = ft.FilePicker(on_result=salvar_no_caminho)

    page.overlay.append(file_picker)  # Adiciona o FilePicker como overlay
    # Layout principal
    page.add(
        ft.Container(
            width=350,
